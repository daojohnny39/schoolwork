    ...
    const ItemType& target) const
{
    ...
    if (subTreePtr != nullptr)
    {
        ...
        else
        {
            ...
            {
                returnPtr = findNode(subTreePtr->getLeftChildPtr(), target);
            }
            else
            {
                returnPtr = findNode(subTreePtr->getRightChildPtr(), target);
            }
        }
    }
...
std::shared_ptr<BinaryNode<ItemType>> BinarySearchTree<ItemType>::removeNode(std::shared_ptr<BinaryNode<ItemType>> nodePtr)
{
    ...
    {
        returnPtr = nodePtr->getRightChildPtr();
    }
    else if (nodePtr->getRightChildPtr() == nullptr)
    {
        returnPtr = nodePtr->getLeftChildPtr();
    }
    else
    {
        std::shared_ptr<BinaryNode<ItemType>> curPtr = nodePtr->getRightChildPtr();
        while (curPtr->getLeftChildPtr() != nullptr)
        {
            curPtr = curPtr->getLeftChildPtr();
        }
        ItemType backup = curPtr->getItem();
        remove(backup);
        nodePtr->setItem(backup);
        returnPtr = nodePtr;
    }
