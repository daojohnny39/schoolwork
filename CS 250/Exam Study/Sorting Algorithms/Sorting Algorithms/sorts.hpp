...
int findIndexOfSmallest(const ItemType theArray[], int start, int size)
{
    int indexSoFar = start; // Index of smallest entry found so far
    for (int currentIndex = start + 1; currentIndex < size; currentIndex++)
    {
        // At this point, theArray[indexSoFar] < all entries in
        // theArray[currentIndex..size-1]
        if (theArray[currentIndex] < theArray[indexSoFar])
            indexSoFar = currentIndex;
    }  // end for
    return indexSoFar; // Index of smallest entry
...
void selectionSort(ItemType theArray[], int n)
{
    ...
    for (int front = 0; front < n - 1; front++)
    {
        ...
        int smallest = findIndexOfSmallest(theArray, front, n);
